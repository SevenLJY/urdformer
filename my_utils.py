...
from my_refs import semantic_color_ref, graph_color_ref, joint_color_ref, sem_ref, joint_ref
...
def prepare_meshes(info_dict):
    ...
    for node in tree:
        ...
        if node["id"] == root_id or node["parent"] == root_id:  # no transform
            ...
            axis_d_p = np.array(
                tree[parent_id]["joint"]["axis"]["direction"], dtype=np.float32
            )
            axis_o_p = np.array(
                tree[parent_id]["joint"]["axis"]["origin"], dtype=np.float32
            )
            bb_0, bb_1 = get_bbox_mesh_pair(
                box_cen,
                box_size,
...
def draw_boxes_axiss_anim(aabbs_0, aabbs_1, axiss, resolution=256):
    '''
