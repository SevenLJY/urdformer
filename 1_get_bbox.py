...
def get_part_bbox(model_id, img_id, root):
...
def prepare_gt_bbox(args):
    ...
    for f in tqdm(os.listdir(input_path)):
        if f.endswith(".png"):
            ...
            img_id = tokens[2]
            bboxes = get_part_bbox(model_id, img_id, args.data_root)
...
def evaluate(args, detection_args):
    ...
    post_processing(label_dir, input_path, args.save_dir)
...
def main():
    ...
    parser.add_argument("--manual_dir", type=str,  default="test_data/labels_gt", help="labels_gt path")
    parser.add_argument("--save_dir", type=str,  default="test_data/labels_filtered", help="labels_filtered for exporting path")
