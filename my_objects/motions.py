...
from my_objects.dict_utils import get_base_part_idx
...
def transform_all_parts(part_vertices, obj_dict, joint_state, 
    ...
    for i in part_visit_order:
        ...
        if not use_plucker: # Direct translation and rotation
            ...
            plucker_direction = np.array(joint["axis"]["plucker"], dtype=np.float32)[:3]
            plucker_moment = np.array(joint["axis"]["plucker"], dtype=np.float32)[3:]
